...
class FileAnalysisTool:
    ...
    def generate_ai_analysis(
        ...
        if self.ai_agent:
            try:
                def _ai_stream(chunk: str) -> None:
                    if stream_callback:
                        stream_callback(prefix + "\n" + chunk)

                if stream_callback:
                    _ai_stream._cancel_event = getattr(
                        stream_callback, "_cancel_event", None
                    )
                ai_analysis = self.ai_agent.analyze_file_with_ai(
                    result, user_question, stream_callback=_ai_stream if stream_callback else None
                )
                if ai_analysis:
